...
function mo_saml_login_validate(){
	...
	if( array_key_exists('SAMLResponse', $_POST) && !empty($_POST['SAMLResponse']) ) {
		...
		if($relayState=='displaySAMLResponse'){
            ...
            $saml_logger->add_log(mo_saml_error_log::showMessage('LOGIN_WIDGET_SAML_STATUS',array('status'=>$status,'statusMessage'=>$statusMessage)), MoSAMLLogger::DEBUG);
		}
		if($status!="Success"){
